    ...
    while(!feof(infPtr))
    {
      ...
      do {
        ...
        if(strcmp("if",tokPtr)==0)
        {
          ...
          {
            sml[smlLoc1]=symbolTable[branchT].location+LOAD*100;
            smlLoc1++;
          }
          tokPtr=strtok(NULL," ");
          if(strcmp("==",tokPtr)==0)
          {
            ...
            if((branchT=searchSymbolLoc(temp1,symbolTable))!=-1)
            {
              sml[smlLoc1]=symbolTable[branchT].location+BRANCHZERO*100;
              smlLoc1++;
            }else{
          ...
          if(strcmp("goto",tokPtr)==0)
          {
            ...
            if((branchT=searchSymbolLoc(temp1,symbolTable))!=-1)
            {
              sml[smlLoc1]=symbolTable[branchT].location+SUBTRACT*100;
              smlLoc1++;
            }
          }
        }
